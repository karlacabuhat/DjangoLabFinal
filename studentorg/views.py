...
class OrganizationList(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if query:
            qs = qs.filter(
                Q(name__icontains=query) |  # Look for name field
                Q(description__icontains=query) |
                Q(college__college_name__icontains=query)  # Look for college_name field
            )
        return qs
...
class OrgMemberDeleteView(DeleteView):
    model = OrgMember
    template_name = 'orgmem_delete.html'
    success_url = reverse_lazy('orgmember-list')
...
class StudentList(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if query:
            qs = qs.filter(
                Q(lastname__icontains=query) |       # Filter based on student's lastname
                Q(firstname__icontains=query)        # Filter based on student's firstname
            )
        return qs
